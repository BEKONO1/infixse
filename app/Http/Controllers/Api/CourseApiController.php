...
class CourseApiController extends Controller
{
    ...
    {
        $categories = Category::take(10)->get()->map(function ($category) {
            return [
                'id' => (int)$category->id,
                'name' => (string)$category->name,
                'course_count' => (int)$category->courseCount,
            ];
        });
    ...
    {
        $courses = Course::where('type', '2')->where('status',1)->where('scope', 1)->latest('total_enrolled')->take(10)->get();
    ...
    {
        $courses = Course::where('type', '3')->where('status',1)->where('scope', 1)->latest('total_enrolled')->take(10)->get();
